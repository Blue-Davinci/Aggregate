...
func (app *application) adminGetAllScraperErrorLogs(w http.ResponseWriter, r *http.Request) {
	...
	qs := r.URL.Query()
	//get the error type
	input.ErrorType = app.readString(qs, "error_type", "")
	...
	errorLogs, metadata, err := app.models.ErrorLogs.GetAllScraperErrorLogs(input.ErrorType, input.Filters)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
...
func (app *application) adminGetAllFeedsWithStatistics(w http.ResponseWriter, r *http.Request) {
	...
	// write the data and metadata
	err = app.writeJSON(w, http.StatusOK, envelope{"feeds": feeds, "metadata": metadata, "statistics": adminFeedStats}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
	}
}
...
func (app *application) adminGetFeedsPendingApprovalHandler(w http.ResponseWriter, r *http.Request) {
	...
	// get the feeds
	feeds, metadata, adminPendingFeedStats, err := app.models.Admin.AdminGetFeedsPendingApproval(input.Name, input.Feed_Type, input.Filters)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
	// write the data and metadata
	err = app.writeJSON(w, http.StatusOK, envelope{"feeds": feeds, "metadata": metadata, "statistics": adminPendingFeedStats}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
	}
}
